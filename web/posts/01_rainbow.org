...
~TODO~ unify angle nomenclature

View angles above the horizon are $\theta_p = 39.3$ deg and $\theta_r
= 44.7$ deg, computed the known light-eye droplet reflection angle
$42$ and the sun and reflection elevations $+-2.7$. Reflection
triangle angles are $A=2.7$, $B=135.3$ and $C=42$.
...
we isolate $d_r = a$ and obtain the distance range
\[
d_r = c \frac{\sin(A)}{\sin(C)} =
[10,16.5] \times 0.0704 = [0.7,1.16] \text{km}
\]

and using $d_r$ as a right triangle hypotenuse we obtain the height range
