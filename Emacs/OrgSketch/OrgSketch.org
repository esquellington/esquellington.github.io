...
- MELPA howto?
- Github + Twitter
*** TODO Error reporting when tools fail
**** TODO Convert tool not available in specified path
- Path may be global, just check it's there and fail if not
**** TODO Selected TOOL not available in specified path
- Path may be global, just check it's there and fail if not
...
*** TODO Detect available TOOLs and choose "best", in order
