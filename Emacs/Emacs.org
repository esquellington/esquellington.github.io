...
- Write keybinding that just does that, on beardbolt-compile open bb-asm and
  split vertically
**** TODO Keep code interleaved with asm?
- objdump can do that!

*** TODO CONTRIBUTE?
- Maybe I could modify + contrib to it?
- Customizations should all be available as local-vars
**** TODO [#A] Option to use flycheck-gcc-include-path automatically for GCC
- Would ensure that if we have FM/FC working on a given project then beardbolt
  works out of the box, which is nice
***** TODO Could add a beardbolt-gcc-include-path var and customize that
- makes sense on its own
- defaults to nil
- easy to bind it to 'flycheck-gcc-include-path, make it one of the available options
**** TODO [#A] Option to avoid redefining C-c C-c in beardbolt-mode
