...
**** TODO [#A] Additonal info on ASM buffer modeline                :EASY:
...
***** TODO CAN reeval modeline lighter with :eval, see SRC mode
...
- Regular narrowing works on SRC and ASM buffers, but it's not synchronized, we
  ...
  toggle narrow/widen
**** TODO [#B] Docs on ASM instruction under point in ASM buffer --> Just use x86-lookup :EASY:
...
**** TODO [#B] Select arch+instruction set for code generation      :EASY:
