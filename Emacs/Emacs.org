...
- Open with [[shell:sh /opt/clion-*/bin/clion.sh &]]
- Open project [[shell:sh /opt/clion-*/bin/clion.sh ~/Desktop/esquellington/tot/Trencadis/ &]]
- Open project [[shell:sh /opt/clion-*/bin/clion.sh ~/Desktop/esquellington/tot/Trencadis/ && xdotool getactivewindow]]
...
** TODO [#A] Command to open current Emacs project in CLion
...
*** TODO Idea: Change focus to CLion and press "Debug" button automatically
...
**** TODO Clicking on icon (XClicker can automate clicks)
- But requires accurate coords, messy and unstable
- See https://ubuntuhandbook.org/index.php/2022/03/automate-mouse-clicks-ubuntu/
- xdotool too, looks better
**** TODO Pressing keyboard shortcut Shift+F9
- Likely there's a way to send X keyboard presses to another app
- See https://superuser.com/questions/518907/command-line-way-to-send-keystrokes-to-a-window-open-on-a-different-x-session
**** xdotool
...
#+BEGIN_SRC elisp :results silent
  (defun clion-debug-current-project ()
    ...
    (let (cmd)
      ...
      (shell-command cmd)))
#+END_SRC
** TODO [#B] Command to add/remove breakpoint at current position
