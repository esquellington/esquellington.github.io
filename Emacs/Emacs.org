...
**** DONE [#A] Toggle automatic compilation on buffer change on/off  :MID:
...
**** DONE [#B] Additonal info on ASM buffer modeline                :EASY:
***** DONE Optimization level -O0..3
- Read from corresponding SRC buffer?
***** DONE Arch
***** DONE CAN reeval modeline lighter with :eval, see SRC mode
**** DONE [#B] Docs on ASM instruction under point in ASM buffer --> Just use x86-lookup :EASY:
...
**** DONE [#A] Keyb to hide bb-compilation buffer                   :EASY:
...
**** DONE [#A] Split bb-execute into args+auto                       :MID:
...
**** DONE HUGE conundrum with local vars! --> forget setq-local, always customize globals
...
**** DONE [#A] ASM -march does not seem to show buffer local vars?   :MID:
- see test.cpp
