...
- FUCK there's a fork that seems even better for C++, but not active and not on
  MELPA
...
*** TODO beardbolt advantages
- Author is also the author of Eglot, so probably knows what they do
**** Code is beardbolt is a lot simpler
- Looks like a stripped-down version, code is similar but shorter
**** Faster, according to github page
**** Does not save buffer on compilation, major issue IMHO
