...
- Main use-case is debugging in CLion, similar to VS debugger, much
  less painful than gdb/cgdb/gud
...
- "jetbrains" mode exists, but it just opens project/file in a move
  verbose way, so why bother
  - See https://github.com/emacs-php/jetbrains.el/blob/master/jetbrains.el
- Would be useful to support customization vars and mode-local vars
  (CLion window id)
