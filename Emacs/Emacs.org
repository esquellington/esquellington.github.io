...
*** Default uses plain gcc or compile_commands.json if exists
...
**** TODO FORK, not Clone, official beardbolt into local /repo
...
**** TODO [#A] Additional beardbolt-gcc-include-flags custom var
- Easy peasy
***** TODO Allow binding to 'flycheck-gcc-include-path to reuse setup?
- Ideally offer this as an option, ex 'flycheck-gcc-include-path, and if
  selected, just query that var and use it
- Would ensure that if we have FM/FC working on a given project then beardbolt
  works out of the box, which is nice
- BUT 'flycheck-gcc-include-path is a list of paths, not a single flags string
**** TODO [#B] Additional beardbolt-gcc-args, like 'flycheck-gcc-args
- Could use these to customize -march, etc..., all except optimization level,
  that goes into next one
**** TODO [#A] Additional beardbolt-gcc-optimization-flags
- Easy to concat
- Thes could be set as local from keybinding?
- Add to bb-mode-map (C-c 0..3?)
- Recompile when changed from keybinding, just call beardbolt-compile automatically
***** TODO [#A] Option to change optimization level in a beardbolt-mode buffer using keybinding
- Just "set opt level to 0..3"
- Ideally as a LOCAL var, but maybe global?
**** TODO [#A] Additonal info on ASM buffer modeline
***** TODO Optimization level -O0..3
***** TODO What else?
**** TODO [#A] Option to disable rainbow completely
**** TODO [#A] Option to avoid default keybindings
...
- Regular narrowing works on SRC and ASM buffers, but it's not synchronized, we
  ...
  toggle narrow/widen
**** TODO [#C] Function to set Layout, port idea from compiler-explorer
