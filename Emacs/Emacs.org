...
- git clone --depth 1 (to get last revision, not full history)
...
- magit-status (C-x g), and then:
  ...
  - 'R' to Rename a file
* Emacs internals
- Interesting blog https://thecloudlet.github.io/blog/project/emacs-01/
* Pico8-mode
- Consider custom mode for easy code section narrowing of .p8 files,
  syntax highlight, re-launching game, etc...
- NO, too much work... probably already exists!?
* proced
- Process viewer, very cool, better than PS and TOP
...
* TODO [#A] emacs Setup Linux env from scratch
- Org file with sequential (sub)-sections to install all stuff I need
  on a Linux PC to
  - Write
    - Emacs
  - Dev
  - Etc
- Should run in vanilla Emacs, could install any Emacs stuff needed first?
- Should assume clean Ubuntu install, but could have sections for
  SteamDeck or others. Could test in a virtual machine with clean install
- Anonymous, in case I want to upload/share it, shouldn't give away
  any personal details, especially passwords
- Ensure (shell) commands run in proper terminal as if they were typed
  in manually
* TODO [#A] LSP
...
** TODO installing clangd (easy)
- https://clangd.llvm.org/installation.html
- Tried on <2026-04-01 mié>, it seems to work but includes fail by
  default and require a ton of configuration to get right, sigh
*** GOOD Requires passing compilation flags
- Either compile_flags.txt or compile_commands.json
- Ah but the .txt doesn't work well enough, no point
*** TODO compile_commands.json seems the best option
- But must be generated, seems very verbose, and no official tool for Makefile
...
* TODO [#C] Emacs * GPT https://github.com/karthink/gptel
...
* TODO [#C] Flycheck or Flymake?
- Make is the default!?
- eglot only supports Flymake?
* TODO Debugging in Emacs
** GUD
...
* GOOD Sketches in org-mode file:OrgSketch/OrgSketch.org
* DONE Latex in comments file:LatexInComments/LatexInComments.org
* TODO emacs-clion
- Interop with CLion IDE from Emacs
- Main use-case is debugging in CLion, much less painful than
  gdb/cgdb/gud
- Overall it's a cool idea, and it WORKS, but I don't use it often
  enough to justify spending time on it for now
** How can we can we send commands from Emacs to CLion?
- only commandline it seems?
- see https://www.jetbrains.com/help/clion/opening-files-from-command-line.html#linux
- See https://www.jetbrains.com/help/clion/working-with-the-ide-features-from-command-line.html#toolbox
- Not sure if there's some kind of listener
- Project info is stored in .idea folder
** DONE Shell commands to open CLion
- Open [[shell:sh /opt/clion-*/bin/clion.sh &]]
- Open project [[shell:sh /opt/clion-*/bin/clion.sh ~/Desktop/esquellington/tot/Trencadis/ &]]
- Open file (in current project, if member) [[shell:sh /opt/clion-*/bin/clion.sh --line 32 ~/Desktop/esquellington/tot/Trencadis/tests/TentaclesMode.cpp &]]
** GOOD [#A] Command to open current Emacs project in CLion
- Ideally use projectile "project" or similar, but that's TOT not
  Trencadis and fails to open properly in CLion

#+BEGIN_SRC elisp :results silent
  (defun clion-open-current-project ()
    "Open current project in CLion"
    (interactive)
    (let (cmd)
      (setq cmd (concat "/opt/clion-*/bin/clion.sh" ;;TODO find or customize
                        " "
                        "~/Desktop/esquellington/tot/Trencadis/" ;;TODO get this from current "project"
                        " &")) ;async
      (message cmd)
      (shell-command cmd)))
#+END_SRC
*** TODO I think the (Makefile) CLion project needs to be setup first (and .idea file created)
- So 1st time CLion is opened in this folder it will not know about
  the project, needs to be setup first
** GOOD [#A] Command to open current Emacs buffer in CLion
- Define in emacs-clion lisp mode or similar?
#+BEGIN_SRC elisp :results silent
  ;; CLion only seems to recognize "~/Desktop" file paths, not full
  ;; paths "/home/oscar/ (as produced by (buffer-file-name) or relative
  ;; path "~/Escriptori" (as produced by (abbreviate-file-name), so we
  ;; must (string-replace) to produce a "~/Desktop" path
  ;; IMPORTANT: This only works properly IF project is alwready open in CLion
  (defun clion-open-current-file ()
    "Open current file in CLion"
    (interactive)
    (let (cmd)
      (setq cmd (concat "/opt/clion-*/bin/clion.sh"
                        (format " --line %d --column %d "
                                (line-number-at-pos nil 1) ;line number, ignores narrowing
                                (current-column)) ;column number
                        (string-replace "Escriptori" "Desktop"
                                        (abbreviate-file-name (buffer-file-name)))))
      (message cmd)
      (shell-command cmd)))
#+END_SRC
** GOOD [#A] Command to Run/Debug in CLion
- Not sure how to do this... there seems to be no commandline option,
  and I don't really understand the scripting/automation API, seems Java?
- Is there a "listener" in CLion that we could send commands to?
- Maybe we'd need a CLion plugin and send commands from Emacs to it?
*** GOOD Idea: Change focus to CLion and press "Debug" button automatically
- Instead of expecting CLion to listen to commands and send them from
  Emacs, we could try to "automate UI events" as if a human performed
  them
**** xdotool
- xdotool can do this it seems https://github.com/jordansissel/xdotool
- NOT in wayland, but who cares
- Get name of focused window
  - [[shell:sleep 5 && cat "/proc/$(xdotool getwindowpid "$(xdotool getwindowfocus)")/comm"]]
- Get active window
  - [[shell:xdotool getactivewindow]]
- Get window by selecting it
  [[shell:xdotool selectwindow]]
  --> returns window ID
- Send "Shift+F9" to specific window ID
  [[shell:xdotool windowactivate 44040276 && xdotool key Shift+F9]]
- Finding CLion actual window ID is not trivial
*** Elisp
#+BEGIN_SRC elisp :results silent
  ;; TODO run this if clion--window-id var is nil, and store it
  ;; there, so that we can use it later in (clion-debug-current-project)
  (defun clion-select-window ()
    "Select CLion window"
    (interactive)
    (let (cmd)
      (setq cmd "xdotool selectwindow")
      (message cmd)
      (shell-command cmd)))

  ;; TODO pass windowid as param or use mode-local var
  (defun clion-debug-current-project ()
    "Debug current project in CLion"
    (interactive)
    (let (cmd)
      (setq cmd (concat "xdotool windowactivate "
                        "52428884" ;TODO windowid for CLion (ex: xdotool selectwindow)
  ...
  (defun clion-close ()
    ...
    (interactive)
    (let (cmd)
      (setq cmd (concat "xdotool windowkill "
                        "52428884")) ;TODO windowid for CLion (ex: xdotool selectwindow)
      (message cmd)
      (shell-command cmd)))
#+END_SRC
** TODO [#C] Command to add/remove breakpoint at current position
- LOL, if I edit the breakpoint definitions in the CLion workspace
  file, they're hot-reloaded by CLion so they change interactively
[[file:~/Escriptori/esquellington/tot/Trencadis/.idea/workspace.xml][breakpoints defined here]]
- We could even show them in Emacs
- Seems very fragile though... sending buffers to CLion and adding
  breakpoints there is probably enough
** TODO [#C] Send file from CLion to Emacs
- Useful for symmetry, but not really necessary
** TODO [#B] Consider minor-mode to add this functionality to C/C++ projects
- "jetbrains" mode exists, but it just opens project/file in a more
  verbose way than mine, see
  https://github.com/emacs-php/jetbrains.el/blob/master/jetbrains.el
- minor-mode would be great to support customization and mode-local
  vars (ex: CLion window id)
- Generalize to any external IDE and call it "puppeteer"?
- Generalize sending keypresses to add new (command,keypress) pairs
  and send them to the IDE using the same method (ex:
  Build,Debug,etc...)
- Simplest minor mode just for clion first, add all existing commands
  and find a cool name
** TODO [#C] VSCode support
- seems feasible, commandline args are quite nice
  https://code.visualstudio.com/docs/configure/command-line
*** [#A] Open (Makefile) project
#+BEGIN_SRC elisp :results silent
  (defun vscodium-open-current-project ()
    "Open current project in VSCodium"
    (interactive)
    (let (cmd)
      (setq cmd (concat "/snap/bin/codium" ;;TODO find or customize
                        " "
                        "~/Desktop/esquellington/tot/Trencadis/" ;;TODO get this from current "project"
                        " &")) ;async
      (message cmd)
      (shell-command cmd)))
#+END_SRC
*** [#A] Open file in project
#+BEGIN_SRC elisp :results silent
  (defun vscodium-open-current-file ()
    "Open current file in VSCodium"
    (interactive)
    (let (cmd)
      (setq cmd (concat "/snap/bin/codium" ;;TODO find or customize
                        " --goto " ;;req to jump to line/col
                        ;;(string-replace "Escriptori" "Desktop"
                        (abbreviate-file-name (buffer-file-name))
                                          ;)
                        (format ":%d:%d"
...
#+END_SRC
*** [#A] Run debugger
*** [#C] Add/Remove breakpoint at point
** TODO [#C] VisualStudio support
- Current VisualBasic method sucks, if there's a commandline way
  without extra code it'd be a lot better
- Otherwise, the minor-mode could just generate the VB script on the
  fly, no need to store it as a separate file and call it, too convoluted
*** [#A] Open file in project
*** [#A] Run debugger
*** [#C] Add/Remove breakpoint at point
*** [#C] Open project
- Not so necessary, it's often open, but may be useful
** DONE Run trencadis from anywhere
- This is now in init.el [[file:~/.emacs.d/init.el::defun project-run (]]
#+BEGIN_SRC elisp :results silent
  (defun run-trencadis ()
    "Run Trencadis"
    (interactive)
    (let (cmd)
      (setq cmd (concat "cd ~/Desktop/esquellington/tot/Trencadis/;"
                        "./trencadis_release &"))
      (message cmd)
      (shell-command cmd)))
#+END_SRC
* org-id
:PROPERTIES:
:ID:       55a5ca56-b964-4fb0-8843-373c8617b5f7
:END:

The (builtin) Elisp func (org-id-get-create) adds a "globally unique" ID to an org header
#+BEGIN_SRC elisp :results silent
  (org-id-get-create)
#+END_SRC

The ID is added as header PROPERTIES, and can be used to link to that
header from any org file (ex [[id:55a5ca56-b964-4fb0-8843-373c8617b5f7]])
or even in code comments or anywhere else (using
org-open-at-point-global)

I'm not sure what "global" means here, or how IDs are found across
directories, likely only org-agenda-files are considered when
searching for ID links, and "global" means unique within the set of
agenda files? --> MM no, I can add ID headers to any org file not in
the agenda list, and they're found from other org files (also not in
the agenda)

Docs are obscure, org-id seems to be part of org-capture/attachments,
but it's not very clear, no good examples. I wonder if anybody has
used this to track "tasks" by ID instead of relying on header text
matching... feels useful to track dependencies as a DAG without
relying on header text/links. If ID are truly global, then they can be
moved to other files, annotated with extra properties, etc
* corfu <2026-04-01 mié>
- Nice completion UI, works oob in elisp and text (ex: org-mode), but
  not in C++, needs a completion backend
- https://github.com/minad/corfu?tab=readme-ov-file
* org mobile
- https://orgro.org/ recomana Richard, sembla simple, val 5 lliures
- https://www.beorgapp.com/ versio prova, funciona pero una mica clunky
