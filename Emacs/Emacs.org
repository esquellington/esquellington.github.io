...
*** Elisp
#+BEGIN_SRC elisp :results silent
  ;; TODO run this if clion--debug-window-id var is nil, and store it
  ;; there, so that we can use it later in (clion-debug-current-project)
  (defun clion-select-debug-window ()
    "Select CLion debug window"
    (interactive)
    (let (cmd)
  ...
  (defun clion-debug-current-project ()
    ...
    (let (cmd)
      (setq cmd (concat "xdotool windowactivate "
                        "8388692" ;TODO windowid for CLion (ex: xdotool selectwindow)
